...
public class PurchaseServiceImpl implements PurchaseService {
        ...
        for (ShoppingCartItem item : this.shoppingCart.getItems()) {
            if (item.isCampaign()) {
                ...
                if (availableCampaigns < item.getUnits()) {
                    throw new ShoppingException("verifyCampaigns() failed for productBundle " + item + " at touchpoint " + this.touchpoint + "! Need " + item.getUnits() + " instances of campaign, but only got: " + availableCampaigns);
                }
            }
        }
    }
    ...
    public void purchase() throws ShoppingException {
        ...
        List<CustomerTransactionShoppingCartItem> productsInCartForTransaction = productsInCart.stream().map(si -> new CustomerTransactionShoppingCartItem(si.getErpProductId(), si.getUnits(), si.isCampaign())).collect(Collectors.toList());
